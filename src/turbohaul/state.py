...
def open_state_db(
    ...
    conn.execute("PRAGMA busy_timeout=5000")  # retry-wait on SQLITE_BUSY
    for stmt in _SCHEMA:
        conn.execute(stmt)
    cur = conn.execute(
...
def audit_db_session(
    ...
    SYNC-ONLY (critical): the threading.Lock blocks the current thread and
...
def reconcile_orphaned_slots(conn: sqlite3.Connection, live_pids: set[int]) -> int:
    ...
    state forever; the new second pass catches them.
    """
    cur = conn.execute(
