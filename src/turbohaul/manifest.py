...
Per ARCHITECTURE.md §8 + §8.1 + §8.2.
Addresses several security hardening concerns: flag injection RCE, tag path
traversal, the lost-update class, and atomic-write durability.

Security-hardening expansion of the flag handling:
...
# === Closed allowlist of safe llama-server flags (§8.1, flag-injection defense) ===
...
SAFE_LLAMA_FLAGS: dict[str, Any] = {
    ...
    # deferred until the validator can parse "N,N,N" safely.
    ...
    "special": bool,
    "spm_infill": bool,
    # === Reasoning — preserved-thinking controls ===
    "reasoning_format": str,    # enum: none/deepseek/deepseek-legacy/auto
    "reasoning": str,           # enum: on/off/auto
    "reasoning_budget": int,    # -1/0/N — preserved-thinking budget knob
...
# Suffix-pattern forward-defense (hardening recommendation).
...
DENIED_FLAGS: set[str] = {
    ...
    "host",
    "port",
    # +22 path-bearing/credential flags (Tom's Fork ships these, were unguarded)
...
# === Tag validation regex (v0.2 §8.1, security-critical) ===
...
def _check_jinja_injection(value: str) -> None:
    """Reject Jinja2 constructs in chat_template body."""
...
def _safe_manifest_path(manifests_root: Path, tag: str) -> Path:
    """Resolve manifest path with realpath check."""
    validate_tag(tag)
...
def write_manifest_atomic(
    ...
    """Atomic write with ETag/If-Match concurrency check (§8.2).
