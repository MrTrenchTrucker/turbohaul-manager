...
These gates mirror Ollama's safety posture so Turbohaul-Manager refuses to
spawn a sidecar when the host cannot safely run it. Each gate is tunable via
RuntimeConfig.queue.safety_*; the all_safety_gates aggregator returns the list
of failures so the manager can surface them on the loading_fail audit +
completion_future error.
...
def check_kv_cache_fit(
    ...
    Note: ``no_kv_offload: true`` is the CANONICAL manifest flag for this — it is
    ...
    if no_kv_offload:
        ...
        # Note: the flat overhead floor does NOT scale with
        ...
        # top of the floor. (A 27B model @256K observed ~2.9 GiB VRAM over body;
        ...
        # Note: if MemAvailable is unreadable this sub-check is skipped, but the
