...
Per ARCHITECTURE.md §10 - addresses these failure modes:
- SIGTERM 5s too short on a 21GB-resident model; no orphan reaper.
- Orphan Popen left running if the parent dies.
- GRACE→POPPED race + drained-SIGTERM.
- Tom's Fork health-contract drift.
...
class SchemaMismatch(RuntimeError):
    """Tom's Fork /health response shape changed unexpectedly (contract-drift defense)."""
...
# Defense-in-depth schema check for Tom's Fork /health.
...
async def drained_sigterm(
    ...
    # Explicit waitpid reap. Without this, the kernel keeps the
...
async def verify_vram_cleared(
    ...
    Defends against the CUDA-allocator-stuck failure mode where VRAM is not freed.
...
def open_and_verify_binary(binary_path: Path, expected_sha256: str) -> int | None:
    """Open the llama-server binary fd at boot + verify sha256 via fd.
