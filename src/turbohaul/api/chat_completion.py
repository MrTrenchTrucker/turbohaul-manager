...
This module ships the non-streaming completion path plus an SSE streaming
pass-through. The manager.submit_and_wait + completion_fn DI is streaming-ready
(just return an async generator from completion_fn and adapt the route).
...
Typed upstream errors: in practice a running container exhibits
...
from turbohaul.live_monitor import compute_generation_id  # live-monitor text tee identity
...
_BODY_MAX_BYTES = 4194304  # 4 MiB total request body cap (oversized-body guard)
...
def _validate_json_schema(rf: dict) -> tuple[bool, str | None]:
    ...
    the dependency being absent (e.g. a writable-layer pip-install that has not
    yet been baked into the image).
...
def _coerce_created_at(created_value: Any) -> Any:
    """Coerce OpenAI int unix-epoch ``created`` into ISO-8601 string for the
...
async def openai_chat_completions(payload: dict, request: Request):
    ...
    if rf is not None:
        ...
        if rf_type == "text":
            ...
            # Validate caller schema; on bad → 422 schema_validation_failed. On
            # ok, response_format propagates via the existing
            # _COMMON_FORWARDED_KNOBS tuple (no separate forwarding path needed).
    ...
    # request.is_disconnected() every 2s; if the client closes, sets the Event
...
_COMMON_FORWARDED_KNOBS = (
    ...
    # size-cap + per-model gating deferred to a later iteration; this is
    # "forward the field to a model that supports tool_calls natively").
    # llama-server mirrors OpenAI's schema, so structured values
...
async def _openai_chat_completions_stream(
    ...
    streaming path: most modern streaming consumers (langchain, Open WebUI,
    OpenAI SDK) parse ``delta.content`` and ``delta.reasoning_content``
    ...
    client_meta = {
        ...
        # is the primary entry for many agent clients, so honoring keep_alive
        # here matters for keeping a hot model resident between turns.
        "keep_alive_s": parse_keep_alive(payload.get("keep_alive")),
    ...
    # wires this but the streaming path historically did NOT, so a streaming
    # slot that sat QUEUED behind a client
    ...
    async def stream_gen():
        ...
        try:
            ...
            gen_id_for_tee = compute_generation_id(
                handle.pid, mgr._spawn_seq, slot.slot_id or slot.thread_id
            )
                ...
                async with client.stream(
                    ...
                    async for chunk_bytes in r.aiter_bytes():
                        if chunk_bytes:
                            ...
                            # happens FIRST, tee SECOND, wrapped fail-open — it can
                            # never delay, reorder, or corrupt the client stream.
                            try:
            ...
            # Tear down the disconnect watcher. Cancel + await so no
...
async def ollama_chat(payload: dict, request: Request) -> dict:
    ...
    # Streaming + tools is deferred to a follow-on iteration. Cheap defensive
    ...
    # Mirror of the OpenAI-path manifest read — slice reasoning_budget for the
    ...
    client_meta = {
        ...
        # the handler-entry validator alone does not propagate the field; this
        # explicit add line keeps the non-stream ollama path honest.
        "response_format": payload.get("response_format"),
        # Manifest reasoning_budget slice for the in-_complete is_thinking_payload
    ...
    if "choices" in result:
        ...
        # string; Ollama clients expect a parsed object. The warning is enriched
...
def _merge_reasoning_into_content(
    ...
    that read only `.content` (many agent workers, langchain default,
    ...
    Guards against an empty-response edge case observed under load with
    ...
    contract that response_format clients depend on. The validate+retry path
    owns the thinking-mode JSON guarantee (blocked upstream on llama.cpp
    #20345 + Ollama #10538); this merger's job is at minimum not to corrupt
    the path.
    """
...
def make_llama_server_complete_fn(
    ...
    async def _complete(slot, handle):
        ...
        # `stream_options`) are deliberately excluded; this path is
        # non-streaming.
        ...
        try:
            async with client_cm as client:
                ...
                # try/except so httpx errors map to the existing
                ...
                if (
                    ...
                    # Lazy import — fail-soft if the dependency is absent.
                    try:
            ...
            # overflow, malformed payload, etc.). Convert to a typed error so
            # the route handler can return 502 Bad Gateway with the upstream
            # status preserved.
            raise SidecarUpstreamError(
            ...
            # Sidecar disconnected / crashed / port closed. Most often KV-cache
            # OOM mid-response, as seen under stress testing. Convert to 503 +
            # Retry-After.
