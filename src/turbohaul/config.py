...
See ARCHITECTURE.md for the security-hardening rationale behind this split.
...
# knob. Bump here if the hardware profile shifts.
...
class QueueConfig(BaseModel):
    ...
    # Default bumped 120 → 300 so multi-turn agents (OpenAI-SDK class) with
    # client-side tool-exec / reflection gaps in the 2-5min range keep their slot
    # warm without needing to send keep_alive. OpenAI-SDK clients can't send
    # keep_alive natively (Ollama Issue #11458).
    # Bumped 300 → 600 because reasoning models with a large reasoning budget on
    # complex compare prompts produce 5-7min client-side inter-turn gaps. The 300s
    # window was eaten by the client's reasoning chain on the FIRST tool-result
    # reflection, not by Turbohaul itself.
