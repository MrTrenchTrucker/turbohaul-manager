"""SSRF guard for pull-url + pull-hf per ARCHITECTURE.md §9.1.

Hardens the model-pull paths against SSRF. Defends against:
  - http/file/ftp/gopher/dict/ldap/data scheme abuse → https-only
  - Internal/metadata endpoints (private services, IMDS) → IP allowlist via denyset
  - NAT64 64:ff9b::/96 + IPv4-compat IPv6 ::/96 bypass classes
