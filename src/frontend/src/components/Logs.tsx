    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-slate-700 bg-slate-950 p-4 text-sm text-slate-400 space-y-3">
        <p>
          ...
          will be added in a polish wave to stream the active sidecar&apos;s stderr (Phase 5+).
          For now, view container logs directly:
        </p>
...
journalctl --user-unit=turbohaul.service -f`}
        ...
        {data && data.active && (
          <div className="text-xs text-slate-400 mt-3">
            ...
            on port{' '}
            <span className="font-mono text-slate-200">{data.active.port}</span>{' '}
            (pid <span className="font-mono text-slate-200">{data.active.pid}</span>)
          </div>
        )}
      </div>
    </div>
