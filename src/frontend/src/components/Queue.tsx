  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-lg border border-slate-700 bg-slate-950 p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500 mb-2">Staging queue</div>
            <div className="text-2xl font-bold text-slate-200 mb-3">
              {queue.staging_queue_depth}
              <span className="text-base font-normal text-slate-500">
                {" "}
                / {queue.staging_queue_max}
              </span>
            </div>
          ...
          <div className="rounded-lg border border-slate-700 bg-slate-950 p-4">
            ...
            <div className="text-2xl font-bold text-slate-200">{queue.acceptance_buffer_depth}</div>
      ...
      <div>
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-950 p-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-mono text-slate-200">{active?.model_tag ?? "—"}</span>
          </div>
          <div className="flex justify-between">
      ...
      <div className="rounded-lg border border-slate-700 bg-slate-950 p-4 text-sm text-slate-400">
        <strong className="text-slate-300">Per-slot detail:</strong> the FIFO list of staged thread
        ids is not yet exposed by /status; W19 will add /api/queue with per-slot rows (position,
        model_tag, thread_id_prefix, state, ETA).
      </div>
