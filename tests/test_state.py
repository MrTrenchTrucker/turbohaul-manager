...
class TestSlotOps:
    def test_upsert_insert_then_update(self, tmp_path):
        with state_db_session(tmp_path / "state.sqlite") as conn:
            ...
            cur = conn.execute("SELECT state, pid, port FROM slots WHERE slot_id=?", ("slot-1",))
            row = cur.fetchone()
    ...
    def test_client_meta_persisted_as_json(self, tmp_path):
        with state_db_session(tmp_path / "state.sqlite") as conn:
            ...
            cur = conn.execute("SELECT client_meta_json FROM slots WHERE slot_id=?", ("slot-2",))
    ...
    def test_known_active_pids(self, tmp_path):
        with state_db_session(tmp_path / "state.sqlite") as conn:
            upsert_slot(conn, {"slot_id": "s1", "model_tag": "m", "state": "ACTIVE", "pid": 100})
            upsert_slot(conn, {"slot_id": "s2", "model_tag": "m", "state": "GRACE", "pid": 200})
            upsert_slot(conn, {"slot_id": "s3", "model_tag": "m", "state": "POPPED", "pid": 300})
    ...
    def test_mark_slot_ended(self, tmp_path):
        with state_db_session(tmp_path / "state.sqlite") as conn:
            ...
            cur = conn.execute("SELECT state, end_reason, ended_at FROM slots WHERE slot_id='s1'")
            row = cur.fetchone()
...
class TestAuditEvents:
    def test_record_event(self, tmp_path):
        with state_db_session(tmp_path / "state.sqlite") as conn:
            ...
            cur = conn.execute("SELECT event_type, payload_json, slot_id FROM audit_events")
            row = cur.fetchone()
    ...
    def test_audit_without_slot_id(self, tmp_path):
        with state_db_session(tmp_path / "state.sqlite") as conn:
            ...
            cur = conn.execute("SELECT event_type, slot_id FROM audit_events WHERE event_type='boot_orphan_reaper'")
            row = cur.fetchone()
...
class TestReconcileOrphanedSlots:
    def test_marks_dead_pids_cold(self, tmp_path):
        with state_db_session(tmp_path / "state.sqlite") as conn:
            upsert_slot(conn, {"slot_id": "s1", "model_tag": "m", "state": "ACTIVE", "pid": 100})
            upsert_slot(conn, {"slot_id": "s2", "model_tag": "m", "state": "ACTIVE", "pid": 200})
            # Only 100 is "alive"
    ...
    def test_marks_pid_null_pre_active_orphans_cold(self, tmp_path):
        ...
        with state_db_session(tmp_path / "state.sqlite") as conn:
            upsert_slot(conn, {"slot_id": "s_staged", "model_tag": "m", "state": "STAGED"})
            upsert_slot(conn, {"slot_id": "s_received", "model_tag": "m", "state": "RECEIVED"})
            upsert_slot(conn, {"slot_id": "s_loading_fail", "model_tag": "m", "state": "LOADING_FAIL"})
            # pre-existing COLD slot must NOT be touched again
            upsert_slot(conn, {"slot_id": "s_cold", "model_tag": "m", "state": "COLD"})
            n = reconcile_orphaned_slots(conn, live_pids=set())
            assert n == 3
            for sid in ("s_staged", "s_received", "s_loading_fail"):
                cur = conn.execute("SELECT state, end_reason FROM slots WHERE slot_id=?", (sid,))
                row = cur.fetchone()
                assert row["state"] == "COLD"
                assert row["end_reason"] == "boot-reconcile-pre-active-orphan"
            cur = conn.execute("SELECT state, end_reason FROM slots WHERE slot_id=?", ("s_cold",))
            row = cur.fetchone()
            assert row["state"] == "COLD"
