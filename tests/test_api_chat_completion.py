...
class TestParseKeepAlive:
    ...
    def test_string_invalid_returns_none(self):
        ...
        # Day/week suffixes intentionally NOT supported (avoids over-engineering;
    ...
    def test_bool_false_means_zero(self):
        ...
        silently fall through to default 300s instead of the immediate teardown
        they asked for.
        """
        from turbohaul.api.chat_completion import parse_keep_alive
...
class TestResponseFormatSUB4a:
    ...
    # (c) openai json_schema body — schema-validation contract.
    # An earlier revision rejected json_schema as deferred (HTTP 400 "not yet
    # supported"). The current behavior validates the schema instead: bad schema
    # → 422 schema_validation_failed. This test guards the 422 contract for the
    ...
    def test_c_openai_json_schema_validation_failed_422(self, app_completion_autostart):
        ...
        assert "json_schema" not in d1["error"]  # error code is generic, not internal
    ...
    # (g) ollama json_schema body — schema-validation contract (mirrors openai).
