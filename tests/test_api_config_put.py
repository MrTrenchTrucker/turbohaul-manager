"""Tests for the PUT /api/config split (runtime vs. boot-only sections)."""
import pytest
...
class TestPutConfigBootForbidden:
    ...
    def test_put_runtime_paths_403(self, app_test):
        """The killer attack: changing runtime.llama_server_binary → RCE primitive (security-critical)."""
        app, client = app_test
        r = client.put(
            "/api/config",
