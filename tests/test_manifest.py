...
class TestManifestSchema:
    ...
    def test_reject_path_bearing_flag_lora(self):
        with pytest.raises(PydanticValidationError, match="explicitly denied"):
            make_manifest(llama_server_flags={"lora": "/root/.ssh/id_ed25519"})
...
class TestParallelManifest:
    """Single-mutator-safe parallelism support: parallel>1 manifest validation
    + argv emission + the ctx-split floor guard."""

    def test_parallel_manifest_parses_and_argv(self):
        # parallel:2 + cont_batching + kv_unified + no_kv_offload parses;
        # ctx_size=32768 splits cleanly into 2x16384 (>= PER_SLOT_CTX_FLOOR).
        m = make_manifest(
            context_size=32768,
            llama_server_flags={
                "ctx_size": 32768,
        ...
        assert m.llama_server_flags["parallel"] == 2
        argv = flags_to_argv(m.llama_server_flags)
        assert "--ctx-size" in argv
        assert "32768" in argv
        assert "--parallel" in argv
        assert "2" in argv
        # Boolean True flags emit as bare --flag (no value).
        assert "--cont-batching" in argv
        assert "--kv-unified" in argv
        assert "--no-kv-offload" in argv

    def test_n_cpu_moe_bool_rejected_int_accepted(self):
        # n_cpu_moe is int-typed: a bool must be rejected (Python bool-is-int
        # coerce), a plain int accepted.
        with pytest.raises(PydanticValidationError, match="expects int, got bool"):
            make_manifest(llama_server_flags={"n_cpu_moe": True})
        m = make_manifest(llama_server_flags={"n_cpu_moe": 4})
        assert m.llama_server_flags["n_cpu_moe"] == 4

    def test_parallel_ctx_validator_rejects_undersized(self):
        # (a) ctx_size not divisible by parallel => rejected. (kv_unified set so
        # the parallel>1 kv_unified requirement passes and we reach the ctx check.)
        with pytest.raises(PydanticValidationError, match="not divisible by"):
            make_manifest(
                context_size=20001,
                llama_server_flags={"ctx_size": 20001, "parallel": 2, "kv_unified": True},
            )
        # (b) ctx_size // parallel below PER_SLOT_CTX_FLOOR (8192) => rejected.
        #     16384 / 4 = 4096 < 8192.
        with pytest.raises(PydanticValidationError, match="PER_SLOT_CTX_FLOOR"):
            make_manifest(
                context_size=16384,
                llama_server_flags={"ctx_size": 16384, "parallel": 4, "kv_unified": True},
            )
        # (c) parallel=1 (or absent) with any ctx_size is unaffected (back-compat).
        m = make_manifest(
            context_size=4096,
            llama_server_flags={"ctx_size": 4096, "parallel": 1},
        )
        assert m.llama_server_flags["ctx_size"] == 4096

    def test_parallel_requires_kv_unified(self):
        # Design #1: parallel>1 WITHOUT kv_unified:true is rejected (the unified
        # KV pool keeps the cache accounting exact + flat across concurrent slots).
        with pytest.raises(PydanticValidationError, match="kv_unified"):
            make_manifest(
                context_size=65536,
                llama_server_flags={"ctx_size": 65536, "parallel": 2},
            )
        # With kv_unified:true it validates (ctx divides cleanly, per-slot >= floor).
        m = make_manifest(
            context_size=65536,
            llama_server_flags={
                "ctx_size": 65536, "parallel": 2, "kv_unified": True,
            },
        )
        assert m.llama_server_flags["parallel"] == 2
