...
A separate bug was discovered immediately after the recovery post-processor was deployed: the OpenAI endpoint's `client_meta` dict (the contract between the route handler and the `_complete` closure) did NOT include `tools` / `tool_choice` / `parallel_tool_calls` / `function_call` / `functions`. This had two effects:
...
A follow-up change adds these five keys to the `client_meta` dict on the OpenAI endpoint, mirroring the `/api/chat` endpoint pattern that already existed from an earlier Ollama tool-call passthrough change:
...
- [CHANGELOG.md](../CHANGELOG.md) `[v0.2.2]` — the earlier Ollama tool-call passthrough on `/api/chat` that established the `client_meta` pattern mirrored by the closure fix
...
- [CHANGELOG.md](../CHANGELOG.md) — `v0.2.3` entry (text-JSON recovery + tools-forwarding closure fix)
