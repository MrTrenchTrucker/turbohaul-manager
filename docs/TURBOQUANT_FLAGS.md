# TurboQuant Flag Recommendations

This document defines the recommended TurboQuant flag set for Turbohaul-Manager manifests targeting the TheTom/llama-cpp-turboquant llama.cpp fork. Apply flags via `PUT /api/manifests/{tag}` with ETag/If-Match atomic concurrency, then verify live via `/proc/<pid>/cmdline` after a cold-spawn. New manifests should ship with these defaults unless a model-specific reason requires deviation.

---

## The five recommended flags

| Flag | Value | Why |
|---|---|---|
| `flash_attn` | `true` | Required for native FP4 MMQ on Blackwell. Was implicit in the older TurboQuant sidecar; explicit in Turbohaul manifests. |
| `no_context_shift` | `true` | Avoids the `shift_context` loop bug that can stall long-context inference. Recommended default — keep enabled. |
...
The five recommended flags above are **spawn argv**. Patching them on a running model requires one of:
...
This is easy to miss: a `/proc/<pid>/cmdline` audit will show the old cmdline still bound on a model that spawned before the manifest PUT. Option B is the least disruptive (wait for natural teardown — typically a few minutes); the next request then spawns with the new flag set, which you can verify live.
...
- Manifest persistence: manifests live as YAML under the manager's manifests directory (e.g. `/var/lib/turbohaul/manifests/`). Bind-mount this directory so manifests survive container restarts.
