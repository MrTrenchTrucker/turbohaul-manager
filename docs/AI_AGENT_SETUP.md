...
Hermes is a representative reference agent. Tested end-to-end with Turbohaul across multi-tool agent loops at sub-2-minute wall.
...
agent:
  ...
  api_max_retries: 240                 # 240 * 30s = 2h ceiling; pair with a 30s retry backoff so cold-load waits ride out a swap
...
- `GET http://<host>:11401/api/tags` → list of available models (Ollama-shape; OpenAI-compatible clients also read this)
...
| "Slot did not reach ACTIVE within 600s" | (Should not happen in current v0.2.2+ builds) | Upgrade to the latest release. This was an earlier bug in the ACTIVE_MATCH streaming path. |
...
| Tool calls fire on Ollama `/api/chat` but not OpenAI `/v1/chat/completions` | Older build silently dropped `tools` on the OpenAI endpoint's `client_meta` | Upgrade to v0.2.3 or later. |
| Tool calls show as text JSON in `message.content`, `message.tool_calls` empty | Either an older build (no recovery layer) OR request did not advertise `tools` (recovery requires the allowlist) | Upgrade to v0.2.3 or later AND ensure your request body includes `tools: [...]`. See [TOOL_CALL_HANDLING.md](TOOL_CALL_HANDLING.md). |
...
| Agent stuck "pondering..." between turns | Inter-turn slot didn't promote via ACTIVE_MATCH | Make sure your model's manifest has `reasoning_budget` capped (recommend 500 for tool-loops). |
| `tools` field rejected with HTTP 400 | Old Turbohaul version without tool-field forwarding | Upgrade to the latest release. |
...
- **Architecture deep-dive:** `ARCHITECTURE.md` (in this `docs/` folder) (queue / slot FSM / IDLE_HOT / ACTIVE_MATCH internals)
...
- **Multi-agent sharing:** [MULTI_AGENT_SHARING.md](MULTI_AGENT_SHARING.md) (running multiple agents against one instance)
