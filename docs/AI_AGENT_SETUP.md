...
[Hermes](https://github.com/<…>/hermes) is the canonical CONTAK fleet agent. Tested end-to-end with Turbohaul up to N≥3-tool agent loops at sub-2-minute wall.
...
docker run -d --name turbohaul-demo \
  ...
  ghcr.io/MrTrenchTrucker/turbohaul-manager:v0.2.2
```
...
- **`/v1/embeddings`**, **`/v1/completions` (FIM)**, **vision content-parts (`image_url`)**, **`response_format` (structured outputs)** — these are all on the v0.2.3 / v0.3 roadmap (Wave 4c/4d/4e/5a/5b/5c). Currently the chat-completions surface is the only validated path.
...
- **Authentication.** v1 fleet-internal port has no auth (`dummy` api_key works). Production-external deployment requires bearer-auth + TLS termination in front (v2 roadmap).
...
- **Recent retrospectives:** `docs/RETRO_*.md` (most recent: Round 8 streaming ACTIVE_MATCH fix, 2026-05-17)
- **Forgejo:** `http://10.244.136.121:3030/matthew/turbohaul-manager` (fleet-internal mirror)
- **GitHub:** `https://github.com/MrTrenchTrucker/turbohaul-manager` (public mirror, possibly behind by a few waves vs Forgejo)
