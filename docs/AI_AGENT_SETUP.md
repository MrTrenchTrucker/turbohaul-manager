...
[Hermes](https://github.com/nousresearch/hermes-agent) is the reference agent used to validate this integration. Tested end-to-end with Turbohaul up to N≥3-tool agent loops at sub-2-minute wall.
...
model_list:
  ...
  - model_name: my-moe-model-turbohaul
    litellm_params:
...
3. **No special handling for thinking models.** Thinking/reasoning models emit `<think>...</think>` in `delta.content` plus structured `delta.reasoning_content`. Your agent can either:
...
| **curator** | **same as main's** — reuse main's `thread_id` | curator restores main's warm prefix read-only to review it, without paying to reprefill (and structurally cannot overwrite main's saved copy) |
...
The rule of thumb: **stable thread = reuse, distinct thread = isolation, shared thread = ride someone's warm cache.** Give every spawned sub-agent its own child `session_id` (the reference integration uses `{parent_session_id}-sub-<nonce>`) so concurrent siblings land in distinct bins — this is what makes context bleed structurally impossible, not just unlikely.
...
- **Curator = main's `thread_id` on purpose.** The curator shares main's thread so it restores main's warm prefix read-only (measured ~26% prefill savings on the reference stack). It can never overwrite main's saved copy — that's structurally enforced (see [TAGS_AND_IDENTITY.md §2](TAGS_AND_IDENTITY.md#2-what-each-role-gets)). There's also an opt-in manager-side route for an even stricter curator-reuses-main path; you don't need it to get the sharing.
...
Each model needs a manifest at `/var/lib/turbohaul/manifests/<model-tag>.yaml`. Example for a 27B thinking model:

```yaml
model_tag: my-model-27b
gguf_path: /var/lib/turbohaul/blobs/my-model-27b-q4_k_xl.gguf
quant: Q4_K_XL
...
- thinking models only preserve `<think>` blocks in the response under `--jinja`
...
| "Slot did not reach ACTIVE within 600s" | (Should not happen in current builds) | Upgrade to the latest release. This was an earlier bug in the ACTIVE_MATCH streaming path. |
...
| Hermes agent stuck "pondering..." | Inter-turn slot didn't promote via ACTIVE_MATCH | Make sure your model's manifest has `reasoning_budget` capped (recommend 500 for tool-loops). |
...
- **`/v1/embeddings`**, **`/v1/completions` (FIM)**, **vision content-parts (`image_url`)**, **`response_format` (structured outputs)** — these are all on the v0.2.3 / v0.3 roadmap. Currently the chat-completions surface is the only validated path.
...
- **Tool-call recovery layer:** [TOOL_CALL_HANDLING.md](TOOL_CALL_HANDLING.md) (wire shape, recovery post-processor, text-JSON case, testing)
