...
**Status:** Validated. Multiple agents on a single host can share one Blackwell GPU via Turbohaul-Manager. A smoke test exercised model-swap serialization across two different models routed from one worker.
...
- Same-model follow-ups inherit the warm process (ACTIVE_MATCH cascade + IDLE_HOT warm-hold, default 10 min).
...
In an example deployment, **two agents default their OpenAI-shape calls to Turbohaul**, and a third agent routes to Turbohaul on a per-task basis:

| Agent | Role | Default LLM backend | Model when routed to Turbohaul |
|---|---|---|---|
| Advisor (27B) | advisory reasoning (27B) | Turbohaul `:11401/v1` | qwen3.6-27b-dense |
| Advisor (35B) | advisory reasoning (35B) | Turbohaul `:11401/v1` | qwen3.6-35b-moe |
| Worker | tool-using worker | cloud NIM (default) | qwen3.6-27b-dense (per-task tool calls) |
...
1. The worker ran a multi-tool task that routed through Turbohaul for qwen3.6-27b-dense inference.
2. The same agent then called the 35B advisor (which defaults to Turbohaul) for advisement.
3. Turbohaul saw the new request for a different model → finalized 27b slot → spawned 35b.
4. The advisor returned a substantive 3-bullet verdict at 85% confidence.
...
Agent C ─┘     │
               ...
               ├─ IDLE_HOT warm-hold, default 10 min (different-thread same-model reuse)
...
- [AI_AGENT_SETUP.md](./AI_AGENT_SETUP.md) — pointing AI agents at Turbohaul as their OpenAI-shape backend.
