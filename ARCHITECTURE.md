...
**Status:** v0.2 design locked 2026-05-16 post-RBSRS Phase 1 critique. Phase 0 ✓. Phase 1 ✓. Phase 2 starts on Cmdr ✓.
**Lineage:** v0.1 (commit d8ceb16) → v0.2 (THIS DOC, post-critique synthesis)
**Owner:** PL Claude (claude-pl) — Cmdr MrTrench directive 2026-05-16.
...
- One-stop-shop for **fleet-internal** local inference (Logbook / GEAR / advisors / Secretary / RELAY / PL). BYOI from consumer side via config-driven URL.

**v0.2 SCOPE NARROWING:** v1 ships **fleet-internal only**. External use (Open WebUI users, untrusted clients) is **explicitly out-of-scope** for v1 and deferred to a v2 hardened build that adds bearer-auth + WS-scoping + TLS termination. This closes the §1-vs-§3 lane mismatch flagged by Security Reviewer #58 finding F9.

## 2. Lineage

- **Replaces** `/mnt/AppData_SSD/Apps/sidecar-manager` (snapshot at Forgejo `matthew/sidecar-manager-archive`, commit `9eda7fa`). Old manager used preempt-based `/ensure` with a single in-process `asyncio.Lock` — IN-PROCESS-only, races with concurrent callers (Secretary vs gear-review) across processes. Documented case: 2026-05-16 Wave 4 E2E hung 240s on this race.
- **Adopts shape from** Ollama (`github.com/ollama/ollama`, MIT, Forgejo mirror `matthew/ollama-mirror`) — API surface only, no source vendored.
- **Embeds binary from** Tom's Fork TurboQuant (Forgejo mirror `matthew/llamacpp-turboquant-mirror`), built in Dockerfile build-stage from pinned SHA.
- **Lives at** Forgejo `matthew/turbohaul-manager`.
...
- **Not external-user-facing.** v1 is fleet-internal. Open WebUI integration deferred to v2.
- Not Ollama Modelfile DSL. Just direct GGUF + per-model yaml.

### 3.1 Singleton Invariant (NEW v0.2 — Brainstormer F1 must-fix)
...
### 3.2 Threat Model & Trust Boundary (NEW v0.2 — Security #58 F9 must-fix)

- **In-scope:** trusted fleet-internal network (CONTAK-01 + ZeroTier mesh + LAN). All callers are known fleet agents (Logbook, Secretary, GEAR, RELAY, advisors, PL).
- **Out-of-scope for v1:** external untrusted users, Open WebUI multi-user deployments, public internet exposure.
- **Bind invariant:** `server.host` defaults to `127.0.0.1`. Operator can explicitly bind to a ZeroTier interface IP via config; binding to `0.0.0.0` requires `--allow-public-bind` CLI flag AND logs a Cmdr-grade warning. **Never `0.0.0.0` by default.**
- **Auth posture:** v1 trusts fleet network. v2 will require bearer-auth on all mutating endpoints. v0.2 doc spec lays the groundwork so v2 can drop in without a structural rewrite.
...
## 6. State Machine per Request (v0.2 — expanded per Brainstormer F2 + Failure Predictor)
...
## 7. Config — Top-Level (v0.2 — split BootConfig vs RuntimeConfig per Security F7 must-fix)
...
## 8. Per-Model Manifest Yaml (v0.2 — ALLOWLIST schema per Security F1 must-fix)
...
### 8.1 Tag Validation + Manifest Write Safety (NEW v0.2 — Security F2 must-fix)
...
### 8.2 Atomic Manifest Writes + Concurrency (NEW v0.2 — Failure Predictor M4 + Brainstormer F4 must-fix)
...
- Out-of-band disk edits (Cmdr SSHes in and edits yaml directly) trigger an inotify watchdog that increments `revision` automatically + writes audit event. FE shows "manifest changed on disk by external editor" notice on next GET. Hot-reload still applies on next stage.
...
### 9.1 Pull-URL Safety (NEW v0.2 — Security F3 must-fix)
...
- Hostname resolved ONCE up front; resulting IP MUST NOT be in:
  ...
  - ZeroTier (`10.244.0.0/16`)
  - NAT64 (`64:ff9b::/96`) — per RC-D7EFD0 bypass class
  - IPv4-compat IPv6 (`::/96`) — per RC-D7EFD0 bypass class
...
### 9.2 Import Safety (NEW v0.2 — Security F4 must-fix)
...
### 9.3 `/status` Response Schema (NEW v0.2 — Brainstormer optional must-have)
...
## 10. Subprocess Management (v0.2 — REWRITE per Brainstormer F3 + Failure Predictor must-fix)
...
### 11.2 FE Rendering Policy (NEW v0.2 — Security F8 must-fix)
...
### 12.1 Blob Lifecycle (NEW v0.2 — Security F5 must-fix)
...
## 13. Migration (v0.2 — REWRITE per Brainstormer F5 + Devil F6 must-fix)

### 13.1 Soak Strategy (Phase 6a — shadow mode)

- Phase 6 ships turbohaul-manager on **port 11401, bound to 127.0.0.1 + ZT IP**. Old manager keeps running on 11400 unchanged.
...
- Cutover edits `secretary_providers.yaml` `pre_call_ensure.url` from `:11400/ensure` to NOTHING (turbohaul queues internally — no /ensure pre-call needed). Logbook + GEAR + advisor route tables update to env-var-driven URL pointing at 11401.
- **Hard cutover deadline:** Day-14 from Phase 6 ship. If turbohaul isn't primary by Day-14, RC /complete-as-failure, roll back. No indefinite parallel-deploy.

### 13.4 Rollback Runbook

- Single command revert: `sed -i 's|11401|11400|g' /etc/secretary_providers.yaml && docker restart logbook-backend && docker restart gear-review`. (Phase 6 deliverable: actual script `/opt/turbohaul/rollback.sh`.)
...
- Tom's Fork TurboQuant llama.cpp = **MIT** (verified on `feature/turboquant-kv-cache:LICENSE`, blob `e7dca554`)
...
- Forgejo repos:
  ...
  - http://10.244.136.121:3030/matthew/llamacpp-turboquant-mirror (analysis copy)
  - http://10.244.136.121:3030/matthew/ollama-mirror (analysis copy)
  - http://10.244.136.121:3030/matthew/sidecar-manager-archive (old manager snapshot)
- Upstream:
...
- Internal SOPs:
  - SOP_Schema_Commit_Gate.md
  - SOP_Deploy_Doctrine.md (uvicorn-reload banned)
  - SOP_Sidecar_Idle_Unload_Watchdog.md (prior art)
  - Solutions/Fleet_Brain_Architecture/Shared/RBSRS_Protocol.md
- Memory:
  - feedback_sidecar_manager_queueing_via_asyncio_lock_intended_20260515.md
  - project_wave4_orphan_thread_fallback_20260516.md
  - project_turbohaul_manager_phase0_design_locked_20260516.md
  - feedback_rbsrs_cold_start_pick_canonical_archetype.md
  - feedback_byte_review_miss_pydantic_future_annotations_20260515.md
  - feedback_pydantic_future_annotations_breaks_fastapi_routes_20260507
- Bypass-class refs (SSRF guard):
  - RC-D7EFD0 (NAT64 64:ff9b::/96 + IPv4-compat IPv6 ::/96 bypass classes)

## 18. Alternatives Considered & Rejected (NEW v0.2 — Devil F1 transparency)

Devil's Advocate (#13) raised two scope-reduction alternatives. Cmdr authority decision 2026-05-16: **rejected both in favor of Turbohaul-Manager full scope per "no tech debt + one-stop-shop" mandate.** Documented here for transparency and to prevent future re-litigation.

### 18.1 Alternative A — `lockd-proxy` (REJECTED)
...
- **Pros:** 1-2 day build vs 3-4 weeks; no new API surface; no FE; no blob store; no auth hardening; no MIT attribution; minimal code review.
- **Cons (Cmdr decision rationale):**
  - **Tech-debt path** — leaves old sidecar-manager in production indefinitely. Cmdr-rejected.
  - Does NOT deliver one-stop-shop local inference. No Ollama compat for future 3rd-party tools.
  - No BYOM blob store. Fleet still depends on host-side model management.
  - No FE observability for queue/sidecar/blob/config.
  - No BYOI fleet-wide capability.
  - Patch-on-patch on the old manager that the doc explicitly says we're moving past.
- **Verdict:** Rejected. lockd-proxy is the tech-debt route Cmdr forbade.

### 18.2 Alternative B — libllama Python bindings (single-process model loading) (REJECTED)
...
- **Cons:**
  ...
  - Tom's Fork TurboQuant innovations are in `llama-server` flags — bindings may lag.
...
### 18.3 Alternative C — `lockd-proxy + Turbohaul concurrent (HYBRID)` (REJECTED)

- **Description:** Build lockd-proxy NOW for immediate fleet-race relief; build Turbohaul-Manager in parallel over 2-3 weeks; cut over to Turbohaul once shipped.
- **Pros:** Immediate relief on existing race; parallel investment.
- **Cons:**
  - Cmdr "no tech debt" — lockd-proxy IS the tech debt. Splitting investment makes both deliverables worse.
...
**End of v0.2 design lock.** All 19 RBSRS Phase 1 findings addressed (13 doc revisions + 6 implementation-baseline shifts). Ready for Cmdr ✓ + Phase 2 implementation start (4-6 days estimate, no tech debt).
